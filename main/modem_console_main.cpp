...
{
    static RTC_RODATA_ATTR char apn_rtc[20] = "internet";
...
#error Invalid serial connection to modem.
   ...
   const ConsoleCommand GetIpAddress("get_ip", "Get the IP address of the PPP interface", no_args, [&](ConsoleCommand * c) {
    if (mode_rtc == esp_modem::modem_mode::DATA_MODE) {  // Ensure the modem is in PPP mode
        esp_netif_ip_info_t ip_info;
        esp_err_t err = esp_netif_get_ip_info(esp_netif, &ip_info);  // Retrieve IP information from the PPP interface
        if (err == ESP_OK) {
            ESP_LOGI(TAG, "IP Address: " IPSTR, IP2STR(&ip_info.ip));
            ESP_LOGI(TAG, "Netmask: " IPSTR, IP2STR(&ip_info.netmask));
            ESP_LOGI(TAG, "Gateway: " IPSTR, IP2STR(&ip_info.gw));
        } else {
            ESP_LOGE(TAG, "Failed to get IP address information: %s", esp_err_to_name(err));
        }
    } else {
        ESP_LOGE(TAG, "Modem is not in PPP mode. Please switch to PPP mode first.");
    }
    return 0;
});

const ConsoleCommand PingGoogle("ping_google", "Ping Google's DNS (8.8.8.8)", no_args, [&](ConsoleCommand * c) {
    if (mode_rtc == esp_modem::modem_mode::DATA_MODE) {  // Ensure the modem is in PPP mode
        esp_ping_config_t config = ESP_PING_DEFAULT_CONFIG();
        config.target_addr.u_addr.ip4.addr = inet_addr("8.8.8.8");
